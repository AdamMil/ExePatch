...
namespace DigitalRune.Windows.TextEditor.Highlighting
{
  ...
  public class DefaultHighlightingStrategy : IHighlightingStrategyUsingRuleSets
  {
    ...
    {
      bool isShaderEffect = (_name == "HLSL" || _name == "Cg"), isAsm = (_name == "Assembly");
          ...
          default:
            {
              ...
              if ((!_inSpan || (_activeRuleSet != null && _activeRuleSet.HighlightDigits)) && (Char.IsDigit(ch) || (ch == '.' && i + 1 < currentLineLength && Char.IsDigit(document.GetCharAt(currentLineOffset + i + 1)))) && _currentLength == 0)
              {
                ...
                {
                  int j;
                  bool sawHex = (ch == '0'), sawSuffix = false;
                  ishex = true;
                  for(j=i; j<currentLineLength; j++)
                  {
                    char c = char.ToUpperInvariant(document.GetCharAt(currentLineOffset + j));
                    if(c >= '0' && c <= '9') continue;
                    else if(c >= 'A' && c <= 'F') sawHex = true;
                    else if(c == 'H')
                    {
                      if(sawSuffix) { ishex = false; break; }
                      else sawHex = sawSuffix = true;
                    }
                    else if(char.IsLetter(c)) { ishex = false; break; }
                    else break;
                  }
                ...
                if (!ishex && ch == '0' && i + 1 < currentLineLength && Char.ToUpper(document.GetCharAt(currentLineOffset + i + 1)) == 'X')
                { // hex digits
              ...
              if (_inSpan)
              {
                if (_activeSpan.End != null && _activeSpan.End.Length > 0)
                {
                  ...
                  {
                    PushCurWord(document, ref markNext, words);
                    string regex = GetRegString(_currentLine, _activeSpan.End, i, document, _activeSpan.IgnoreCase);
              ...
              if (_activeRuleSet != null)
              {
                foreach (Span span in _activeRuleSet.Spans)
                {
                  ...
                  {
                    PushCurWord(document, ref markNext, words);
                    string regex = GetRegString(_currentLine, span.Begin, i, document, _activeRuleSet.IgnoreCase);
    ...
    /// </summary>
    static string GetRegString(LineSegment lineSegment, char[] expr, int index, IDocument document, bool ignoreCase)
    {
      int j = 0;
      ...
      for (int i = 0; i < expr.Length; ++i, ++j)
      {
        ...
        switch (expr[i])
        {
          case '@': // "special" meaning
            ...
            break;
          default:
            char ca = expr[i], cb = document.GetCharAt(lineSegment.Offset + index + j);
            if(ignoreCase)
            {
              ca = char.ToUpperInvariant(ca);
              cb = char.ToUpperInvariant(cb);
            }
            if (ca != cb) return regexpr.ToString();
            regexpr.Append(cb);
            break;
        }
      }
