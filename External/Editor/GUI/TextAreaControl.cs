...
namespace DigitalRune.Windows.TextEditor
{
  ...
  public class TextAreaControl : Panel
  {
    ...
    public void AdjustScrollBars()
    {
      ...
      _vScrollBar.Enabled = _vScrollBar.Maximum > _textArea.TextView.DrawingPosition.Height;
      int maxLineLen = 20;
      ...
      for (int lineNumber = firstLine; lineNumber <= lastLine; lineNumber++)
      {
        ...
        if (Document.FoldingManager.IsLineVisible(lineNumber))
        {
          ...
          {
            maxLineLen = Math.Max(maxLineLen, _lineLengthCache[lineNumber]);
          }
          else
          {
            ...
            maxLineLen = Math.Max(maxLineLen, visualLength);
          }
        }
      }

      _hScrollBar.Minimum = 0;
      _hScrollBar.Maximum = maxLineLen;
      _hScrollBar.Enabled = _hScrollBar.Maximum > _textArea.TextView.NumberOfVisibleColumns-1;
    ...
    public void ScrollTo(int line)
    {
      ...
      if(_textArea.TextView.DrawingPosition.Height != 0 &&
         (line < _textArea.TextView.FirstPhysicalLine + _scrollMarginHeight ||
          line >= _textArea.TextView.FirstPhysicalLine + _textArea.TextView.NumberOfVisibleLines - _scrollMarginHeight))
      {
        int newTop = line - _textArea.TextView.NumberOfVisibleLines/2;
        _vScrollBar.Value = Math.Max(0, Math.Min(_vScrollBar.Maximum - _vScrollBar.LargeChange + 1, newTop * _textArea.TextView.LineHeight));
