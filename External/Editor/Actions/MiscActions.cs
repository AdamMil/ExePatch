...
namespace DigitalRune.Windows.TextEditor.Actions
{
  ...
  {
    /// <summary>Toggles comments on each line in a selection or on the current line.</summary>
    public ToggleLineComment() { }

    /// <summary>Sets or unsets comments on each line in a selection or on the current line.</summary>
    public ToggleLineComment(bool commentOut)
    {
    ...
    bool ShouldComment(IDocument document, string comment, ISelection selection, int startLine, int endLine)
    {
      for (int i = endLine; i >= startLine; --i)
      {
        ...
        if (lineText.Length != 0 && !lineText.StartsWith(comment))
        {
          return true;
        }
      }
    ...
    public override void Execute(TextArea textArea)
    {
      ...
      if (textArea.SelectionManager.HasSomethingSelected)
      {
        bool shouldComment = commentOut.GetValueOrDefault(true);
        if(!commentOut.HasValue)
        {
          foreach(ISelection selection in textArea.SelectionManager.Selections)
          {
      ...
      else
      {
        ...
        if (commentOut.HasValue ? commentOut.Value : ShouldComment(textArea.Document, comment, null, caretLine, caretLine))
